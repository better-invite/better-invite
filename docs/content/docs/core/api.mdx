...
<Callout type="warn" title="Deprecated">
  This endpoint is deprecated. Prefer [Accept Invite](#accept-invite) for authenticated acceptance.
</Callout>

<APIMethod path="/invite/activate" method="POST" optionalSession note="Deprecated. Prefer acceptInvite (session required) or acceptInviteCallback (email links).">
```ts
type activateInvite = {
	/**
...
<APIMethod path="/invite/accept" method="POST" requireSession note="Requires an authenticated session. Unauthenticated email links should use Accept Invite Callback instead.">
```ts
type acceptInvite = {
	/**
	 * Where to redirect the user after accepting the invite.
...
<Callout type="warn" title="Deprecated">
  This endpoint is deprecated. Prefer [Accept Invite Callback](#accept-invite-callback).
  Legacy links used `/invite/:token/activate?callbackURL=...`. New links use `/invite/:token?callbackUrl=...`.
  The new callback also accepts the legacy `callbackURL` query alias.
</Callout>

<APIMethod path="/invite/:token/activate" method="GET" optionalSession fetchOptionsParams="token" note="Deprecated. Prefer acceptInviteCallback.">
```ts
type activateInviteCallback = {
	...
	*/
	...
	/**
	 * Where to redirect the user to sign in/up.
	 * {callbackUrl} will be replaced by the actual callbackUrl.
	*/
	signInUpUrl?: string
	/**
   * @ignore
   * The invite token from the URL path
  */
  token: string
	/**
	 * The email address of the user to sign in/up to.
...
<Callout type="info" title="Note">
  ...
  For authenticated acceptance in your app UI, use [Accept Invite](#accept-invite) instead.
</Callout>
