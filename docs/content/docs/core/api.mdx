...
<APIMethod path="/invite/activate" method="POST" requireSession note="If the user is not logged in, it redirects the user to the callback URL with the token in a cookie.">
```ts
type activateInvite = {
	/**
...
<Callout type="warn" title="Deprecated">
  This endpoint is deprecated, use accept invite instead for POST requests
</Callout>
