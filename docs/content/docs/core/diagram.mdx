...
This diagram shows the full invite acceptance flow, including both logged-in and not logged-in users, with validations, role updates, hook execution, and final redirection.

```mermaid
flowchart TB
  %% Acceptance entry points
  N1(User clicks invite link) --> ACT(acceptInviteLogic)
  N2(Client calls accept API) --> ACT
