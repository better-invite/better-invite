...
export function Banner({
	...
	/**
	 * FOr rainbow variant only, customise the speed, the lower, the faster
	 *
	 * @default 20s
	 */
	rainbowSpeed?: string;
	...
	return (
		...
		>
			...
			{globalKey ? (
				<script
					// biome-ignore lint/security/noDangerouslySetInnerHtml: it's the only way to hide the banner without having to wait to load the local storage
...
function flow({ colors, speed }: { colors: string[]; speed: string }) {
	return (
		<>
			<div
				...
				style={
					{
						...
						animation: `fd-moving-banner ${speed} linear infinite`,
